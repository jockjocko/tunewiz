...
const SidebarLink: FC<SidebarLinkProps> = ({ text, Icon, url }) => {
    return (
...
export const Layout: FC<LayoutProps> = ({ children, links }) => {
    ...
    const drawer = (
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <ListItem sx={{ p: 2, display: 'flex', justifyContent: 'center', alignItems: 'center' }} button component={Link} to={'/'}>
                <img height={38} alt="Squeegee" src={logo} />
            </ListItem>
            <Divider />
            <Box sx={{ flex: 1 }}>
                <List>
    ...
    return (
        <Box sx={{ display: 'flex', height: '100%' }}>
            ...
            <Box component="main" sx={{ height: '100%', flexGrow: 1, width: { sm: `calc(100% - ${drawerWidth}px)` } }}>
                <Toolbar />
                {children}
            </Box>
        </Box>
